...
async def test_savings_rate_trend_returns_latest_available_monthly_totals(client, monkeypatch):
    """Savings-rate trend returns compact monthly base-currency income and expense totals."""
    ...
    async with TestSession() as session:
        session.add_all([
            ...
            _transaction(user_id, usd_account_id, groceries_id, date(2026, 5, 2), -900_000, "USD"),
        ])
        await session.commit()
...
async def test_savings_rate_trend_returns_empty_payload_without_base_currency_accounts(client, monkeypatch):
    """Users without visible base-currency accounts receive an empty payload."""
    from app.routes import insights as insights_routes
    ...
    async with TestSession() as session:
        session.add_all([
            ...
            _transaction(user_id, usd_account_id, salary_id, date(2026, 5, 1), 700_000, "USD"),
        ])
        await session.commit()
