...
app_version="$(read_env_var "$dev_dir/.env" APP_VERSION)"
app_image_tag="$(read_env_var "$dev_dir/.env" APP_IMAGE_TAG)"

: "${app_version:?APP_VERSION is required in dev/.env}"
: "${app_image_tag:?APP_IMAGE_TAG is required in dev/.env}"
