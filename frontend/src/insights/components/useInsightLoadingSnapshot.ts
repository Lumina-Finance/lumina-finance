...
type UseInsightLoadingSnapshotOptions<T> = {
  snapshot: T
  loading?: boolean
...
type InsightLoadingSnapshotState<T> = {
  displaySnapshot: T
...
export function useInsightLoadingSnapshot<T>({
  snapshot,
  loading = false,
  transitionKey,
  minVisibleMs = DEFAULT_LOADING_MIN_MS,
}: UseInsightLoadingSnapshotOptions<T>): InsightLoadingSnapshotState<T> {
