...
@pytest.mark.parametrize("raw_amount", ["$12.34", "1,234.56", "1,234", " 99.99 "])
def test_parse_import_amount_to_minor_units_rejects_malformed_text(raw_amount):
    """Import amount parsing raises a 422 error for malformed amount text"""
    with pytest.raises(HTTPException) as exc_info:
        parse_import_amount_to_minor_units(raw_amount, CAD)

    assert exc_info.value.status_code == 422
    assert exc_info.value.detail == f"Invalid amount: {raw_amount}"
...
def test_parse_import_amount_to_minor_units_preserves_generated_minor_units(
    ...
    """Normalized amounts preserve generated minor units while padding is refused"""
    ...
    for padded_amount in (f" {raw_amount}", f"{raw_amount} ", f"{raw_amount}\n"):
        with pytest.raises(HTTPException) as exc_info:
            parse_import_amount_to_minor_units(padded_amount, currency)
        assert exc_info.value.status_code == 422
        assert exc_info.value.detail == f"Invalid amount: {padded_amount}"
